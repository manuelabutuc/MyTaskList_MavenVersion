...
function listTasks(tasks) {
    ...
    for (var i = 0; i < tasks.length; i++) {
        ...
        var taskHtml =
            ...
            '<input type="checkbox" style="width:20px; height:20px; vertical-align:bottom; border-radius:0.25em; margin-left:5px; margin-right:5px; position:relative; bottom:-1px"' +
            ' value="' + task.id + '" onclick=markDone("' + task.id + '")>' +
            task.name +
            '</li>';
        listHtml += taskHtml;
    }
